...
3. 移除数组中值为val的元素：https://leetcode-cn.com/problems/remove-element/
...
3. 在排序的数组里面找到target的第一个和最后一个元素：https://leetcode-cn.com/problems/find-first-and-last-position-of-element-in-sorted-array/
...
3. 所有的组合数的和：https://leetcode-cn.com/problems/combination-sum/
   ...
   class Solution {
       ...
       void dfs(int candidates[],int target,List<Integer> l,int index){
           ...
           for(int i=index;i<candidates.length;i++){
               if(target>=candidates[i]){
                   l.add(candidates[i]);
                   dfs(candidates,target-candidates[i],l,i);//对
...
4. 组合总和2：https://leetcode-cn.com/problems/combination-sum-ii/
   ...
   class Solution {
       ...
       void dfs(int candidates[],int target,List<Integer> l,int index){
           ...
           for(int i=index;i<candidates.length;i++){
               if(i>index&&candidates[i]==candidates[i-1]){//快速去重方法，遇到重复的元素就先continue一下
                   continue;
               }
               if(vi[i]==0&&target>=candidates[i]){
                   l.add(candidates[i]);
                   vi[i]=1;
                   dfs(candidates,target-candidates[i],l,i+1);//对
                   l.remove(l.indexOf(candidates[i]));
                   vi[i]=0;
               }
           }
...
5. 缺失的第一个正数：https://leetcode-cn.com/problems/first-missing-positive/

   原地置换，但是记得要用while，因为换了一次之后也不一定是对的，所以要再换一次，这里面数字的关系需要找清楚再做后面的计算。
...
1. 接雨水：https://leetcode-cn.com/problems/trapping-rain-water/
   ...
   找到从当前位置到左右的分别的最大值，然后每次就是用当前左右的最大值减去当前的值，就是我们可以接的雨水了。
   ...
   class Solution {
       public int trap(int[] height) {
           ...
           for(int i=0;i<height.length;i++){
               r+=Math.min(rMax[i],lMax[i])-height[i];
           }
           return r;
       }
   }
       ...
       public String multiply(String num1, String num2) {
           ...
           for(int i=num1.length()-1;i>=0;i--){
               ...
               for(int j=num2.length()-1;j>=0;j--){
                   ...
                   arr[i+j+1]+=x*y//这是重点，i*j是第i+j+1位
               }
           }
           for(int i=arr.length-1;i>0;i--){//进位处理
               arr[i-1]+=arr[i]/10;
               arr[i]%=10;
           }
           String s="";
           ...
           return arr[0]==0?s.substring(1):s;
       }
   }
   ```

3. 跳数组：https://leetcode-cn.com/problems/jump-game-ii/
   ...
   class Solution {
       public int jump(int[] nums) {
           int len=nums.length;
           if(len<=1) return 0;
           int maxPo=0,steps=0,nowStep=0;
           for(int i=0;i<len-1;i++){//贪心算法，在找得到的目标里面找最远的那个
...
1. 全排列：https://leetcode-cn.com/problems/permutations/
   ...
   class Solution {
       ...
       public List<List<Integer>> permute(int[] nums) {
           ...
           int len=nums.length;
           vi=new int[len];
           dfs(nums,new ArrayList<>(),len);
           return ll;
       }
       void dfs(int nums[],List<Integer> l,int target){
           if(target==0){
               List<Integer> temp=new ArrayList<>();
               temp.addAll(l);
               ll.add(temp);
               ...
               for(int i=0;i<target;i++){
                       l.add(nums[i]);
                       int tmp = nums[i];//每次把访问过的交换到后面，那么就可以复杂度降低
...
1. 全排列，有重复：
   ...
   class Solution {
       ...
       void dfs(int nums[],List<Integer> l,int n){
           if(n==nums.length){
               ll.add(new ArrayList<>(l));
               return ;
           }
           for(int i=0;i<nums.length;i++){
               if(vi[i]||(i>0&&nums[i]==nums[i-1]&&vi[i-1]==false)){//去重
                   continue;
               }
               l.add(nums[i]);
               vi[i]=true;
               dfs(nums,l,n+1);
               vi[i]=false;
               l.remove(n);//删除的时候因为有重复所以去除index的方法是错误的
           }
       }
   }
   ```
   ...
   class Solution {
       ...
       void dfs(int n,List<Integer> l,int len){
           if(n==len){
               List<Integer> temp=new ArrayList<>();
               temp.addAll(l);
               ll.add(temp);
           ...
           for(Map.Entry<Integer, Integer> entry : m.entrySet()){
               if(entry.getValue()!=0){
                   ...
                   dfs(n+1,l,len);
                   l.remove(n);//问题在于删除是按下标删除所以重复的处理得不好
                   m.put(entry.getKey(),entry.getValue()+1);
               }
           }
       }
...
1. 矩阵的旋转：https://leetcode-cn.com/problems/rotate-image/submissions/
...
3. pow(x,y)：https://leetcode-cn.com/problems/powx-n/

   之前做过这题，递归完美解决，而且是快速幂乘法
...
4. n皇后问题：https://leetcode-cn.com/problems/n-queens/

   大一困扰我的问题，现在终于凭借一己之力写出来了
   ...
   class Solution {
       ...
       boolean check(int row,int col,int n){
           ...
           for(int i=1;i<n;i++){
               if(row-i>=0&&col-i>=0&&queen[row-i][col-i]) return false;
               if(row-i>=0&&col+i<n&&queen[row-i][col+i]) return false;
           }
           return true;
       }
...
1. 八皇后问题2，返回数量：https://leetcode-cn.com/problems/n-queens-ii/
...
2. 最大连续子序列和：https://leetcode-cn.com/problems/maximum-subarray/

   忘了之前做过的题，就是一个dp关系，经典题目：
   ...
   这样做一点忘一点，前面的都忘了，做了也没用。

### 4.10 leetcode

1. 旋转矩阵，做过的题：https://leetcode-cn.com/problems/spiral-matrix/
   ...
   class Solution {
       public List<Integer> spiralOrder(int[][] matrix) {
           ...
           while(left<=right&&top<=down){
               ...
               right--;
               for(i=right;i>=left&&top<=down;i--){//这里要判断上面是否小于下面，不能直接做
                   l.add(matrix[down][i]);
               }
               down--;
               for(j=down;j>=top&&left<=right;j--){//这里要判断left是否小于右边
                   l.add(matrix[j][left]);
               }
               left++;
           }
...
2. 数组跳跃，能否到达最后一个元素：
   ...
   class Solution {
       public boolean canJump(int[] nums) {
           ...
           for(int i=0;i<nums.length;i++){
               if(i>maxDis) return false;//代表i不可达
               maxDis=Math.max(maxDis,i+nums[i]);//每次贪心计算当前能到达的最远
           }
           return true;
       }
   }
...
3. 合并区间：https://leetcode-cn.com/problems/merge-intervals/
   ...
   先排序，然后再扫描，重点是集合的使用方法：

   ```java
   class Solution {
       public int[][] merge(int[][] intervals) {
           Arrays.sort(intervals,new Comparator<int []>(){
               public int compare(int inte1[],int inte2[]){
...
1. 插入区间：和上一题类似，只是现在是插入一个区间再进行合并：https://leetcode-cn.com/problems/insert-interval/
   ...
   class Solution {
       public int[][] insert(int[][] intervals, int[] newInterval) {
           ...
           int ll[][]=list.toArray(new int[list.size()][]);
           ...
           for(int i=0;i<ll.length;i++){
               int l=ll[i][0],r=ll[i][1];
               if(mer.size()==0||mer.get(mer.size()-1)[1]<l){
                   mer.add(new int[]{l,r});
               }
               else{
                   mer.get(mer.size()-1)[1]=Math.max(mer.get(mer.size()-1)[1],r);
               }
           }
   ...
   class Solution {
       public int[][] generateMatrix(int n) {
           ...
           while(l<=r&&t<=d){
               ...
               r--;
               for(int i=r;i>=l&&l<=r;i--){
                   nums[d][i]=++cnt;
               }
               d--;
               for(int i=d;i>=t&&t<=d;i--){
                   nums[i][l]=++cnt;
               }
               l++;
           }
...
2. 全排列的第k个数：https://leetcode-cn.com/problems/permutation-sequence/
   ...
   class Solution {
       ...
       public String getPermutation(int n, int k) {
           ...
           return dfs(n,k,nums,0,0);
       }
       String dfs(int n,int k,int nums[],int index,int sum){
           if(index==n){//说明n个数都选择了
               cnt++;
               if(cnt==k) return String.valueOf(sum);
               else return "";
           }
           else{
               for(int i=1;i<=n;i++){
                   if(!vi[i]){
                       sum=sum*10+nums[i];
                       vi[i]=true;
                       String re=dfs(n,k,nums,index+1,sum);
                       if(re!="") return re; 
                       vi[i]=false;
                       sum=sum-nums[i];
                       sum/=10;
   ...
   就是每次一位数，都是有多个n-1的阶乘这么多个数的，每次都减去相应的数目，找到对应的数字添加到字符串里面
   ...
   class Solution {
       public String getPermutation(int n, int k) {
           ...
           k-=1;
           for(int i=n-1;i>=0;i--){
               int index=k/fac[i];//第i个数的地方有阶乘个这么多的数，每次除以了就知道下标是多少了
               sb.append(ca.remove(index));
               k-=index*fac[i];
           }
           return sb.toString();
       }
...
3. 旋转链表：https://leetcode-cn.com/problems/rotate-list/
   ...
   方法是这样，但是有更简单的，大概，就是可以先找出长度，然后就把首尾接起来，之后再根据相关的k找到头返回就行。
...
4. 找出方格里面不同路径的个数：https://leetcode-cn.com/problems/unique-paths/
   ...
   class Solution {
       public int uniquePaths(int m, int n) {
           int dp[][]=new int[m][n];
           for(int i=0;i<m;i++)//边缘两个都是只有一条路的
               dp[i][0]=1;
           for(int i=0;i<n;i++)
               dp[0][i]=1;
           ...
           for(int i=1;i<m;i++){
               for(int j=1;j<n;j++){
                   dp[i][j]=dp[i-1][j]+dp[i][j-1];//每个当前格子到达的路径数目是左边和上边的和
               }
           }
           return dp[m-1][n-1];
       }
   ...
   还有方法是，肯定走m+n-2步路，其中m-1步走的是向下，所以就是一个组合数的选取：
   ...
   class Solution {
       public int uniquePaths(int m, int n) {
           ...
           for(int x=n,y=1;y<m;y++,x++){
               ans=ans*x/y;//要乘之后再除，否则会有问题，即整数除的问题。
           }
           return (int)ans;//用int会越界，所以用long
       }
   }
   ```
...
5. 有障碍的走迷宫路径数：https://leetcode-cn.com/problems/unique-paths-ii/
...
7. 加1：https://leetcode-cn.com/problems/plus-one/
   ...
   class Solution {
       public int[] plusOne(int[] digits) {
           ...
           int nums1[]=new int[digits.length+1];
           nums1[0]=1;
...
8. 将数组排成一个最大的数：
   ...
   字典序排列，自己写一个排列的方式，s1+s2与s2+s1进行比较，大于的放在前面，这样就可以进行排序

   ```java
   class Solution {
       public String largestNumber(int[] nums) {
           Integer Num[]=new Integer[nums.length];
           for(int i=0;i<nums.length;i++){//包装类型才可以进行sort排序，int只有普通的自定义排序
               Num[i]=nums[i];
           }
           Arrays.sort(Num,new Comparator<Integer>(){
               public int compare(Integer num1,Integer num2){
                   String s1=String.valueOf(num1),s2=String.valueOf(num2);
                   String sc1=s1+s2,sc2=s2+s1;//对两个相加进行比较，找出大的哪一个
                   return -sc1.compareTo(sc2);//逆序排列，大的在前面
               }
           });
           if(Num[0]==0) return "0";//巧妙地去除000情况，因为最大的都是0说明全0
           String r="";
           for(int i=0;i<nums.length;i++){
               r+=Num[i];
           }
...
1. 找到二叉搜索树里面差值最小的两个数：https://leetcode-cn.com/problems/minimum-distance-between-bst-nodes/

   二叉搜索树，差值最小只会出现在根节点和左子树最小以及右子树最小，这种结构一定要想到是中序遍历
...
2. 二进制加法：https://leetcode-cn.com/problems/add-binary/
   ...
   class Solution {
       public String addBinary(String a, String b) {
           ...
           int carry=0;
           for(int i=0;i<len;i++){
               carry+=i>=a.length()?0:(a.charAt(a.length()-i-1)-'0');
               carry+=i>=b.length()?0:(b.charAt(b.length()-i-1)-'0');
...
1. 实现trie树，就是给一个树，可以知道要查询的某个前缀在不在里面：https://leetcode-cn.com/problems/implement-trie-prefix-tree/

   思路就是进行一个26叉树的书写，每次进来的插入都从一个一个字母往下面走。

   ```java
   class Trie {
   
       public class TrieNode{
           public boolean isEnd;//用来表示是否到达一个单词的末尾
       ...
       public void insert(String word) {
           ...
           for(int i=0;i<word.length();i++){
               ...
               if(cur.next[ch]==null){//是null才创建，不是不用，因为之前创建过了
...
2. x的平方根：https://leetcode-cn.com/problems/sqrtx/
   ...
   二分法，但是注意用x/m<m判断m*m>x，因为后一种方法容易溢出，尤其是在数字较大，如Integer.MAX_VALUE
...
1. 盗窃：是一个环，首尾不能同时盗窃，只能盗窃相邻的某一家，所以分为两个数组，一个有第一个元素没有最后一个元素，一个没有第一个元素但是有最后一个元素。然后用动态规划的方法，然后一个一个进行比较，
   ...
   class Solution {
       ...
       int doRob(int nums[],int start,int end){
...
1. 有效路径，字符串处理：https://leetcode-cn.com/problems/simplify-path/
...
1. 解码方法个数：https://leetcode-cn.com/problems/decode-ways/

   一开始dfs超时，是因为做了很多无用的计算，直接动态规划，从后面往前面看，
...
1. 中序遍历成为一条树：https://leetcode-cn.com/problems/increasing-order-search-tree/
...
1. 每天运送包裹的最小能力：https://leetcode-cn.com/problems/capacity-to-ship-packages-within-d-days/

   其实就是运送能力是有左边界和右边界的，我们就用边界进行计算，每次二分需求，各自计算看是否可以运成功
...
2. 编辑距离(hard)：https://leetcode-cn.com/problems/edit-distance
   ...
   因为A的删除和B的插入一样，A的替换和B的替换也是一样的，所以仅有这三种操作。

   对于一个空字符串，到达另一个字符串的路是另一个字符串的长度。

   对于i j，其最多是A添加一个或者B添加一个，或者看两者i-1和j-1是否相同，最多就加1
   ...
   class Solution {
       public int minDistance(String word1, String word2) {
           ...
           for(int i = 1; i< m + 1; i++){
               for(int j = 1; j < n + 1; j++){
                   int plus1 = dp[i][j - 1] + 1;//这是B字符串添加一个即可
                   int plus2 = dp[i - 1][j] + 1;//这是A字符串添加一个即可
                   int alter = dp[i - 1][j - 1];//这是A或者B更改第i-1或者j-1个字符就行
                   if(word1.charAt(i-1) != word2.charAt(j-1)){//如果不相等那就需要更改一个的内容
                       alter += 1;
                   }
                   dp[i][j] = Math.min(alter, Math.min(plus1, plus2));//取他们的最小值
               }
           }
           return dp[m][n];//最后就是看长度为m和n的两者之间的距离是多少
       }
   }
   ```

3. 原地置换数组为0数组：https://leetcode-cn.com/problems/set-matrix-zeroes/

   要求常数空间，所以我们用第一行和第一列进行存储，先看需不需要变成0，再进行一个选取置为0

   ```java
   class Solution {
       public void setZeroes(int[][] matrix) {
           int l = matrix.length, ll = matrix[0].length;
...
4. 搜索二维矩阵，其一行从左到右大，下一行大于上一行最后一个：

   二分搜索的重点：left <= right，每次左=mid+1，右=mid-1；
...
5. 颜色分类：
   ...
   原地更换，重点是怎么更换。其实就是2移到最后，0移到最前面，但是要保证不会重复，所以

   ```java
   class Solution {
       public void sortColors(int[] nums) {
           int dirO = 0, dirT = nums.length - 1;
           for(int i = 0; i <= dirT; i++){//这里的是T
               while(i <= dirT && nums[i] == 2){//对于2要一直置换下去
                   int temp = nums[dirT];
                   nums[dirT] = nums[i];
                   nums[i] = temp;
                   dirT--;
...
1. 在某个范围内的二叉搜索树节点之和：https://leetcode-cn.com/problems/range-sum-of-bst/
...
1. 平方数之和：给一个数，问是不是两个数的平方和：https://leetcode-cn.com/problems/sum-of-square-numbers/
...
1. 有一堆石头，用整数数组 stones 表示。其中 stones[i] 表示第 i 块石头的重量。
   ...
   class Solution {
       public int lastStoneWeightII(int[] stones) {
           ...
           int m = sum / 2;
           boolean dp[][] = new boolean[stones.length + 1][m + 1];
           dp[0][0] = true;
   ...
   <img src="算法学习.assets/image-20210608210154621.png" alt="image-20210608210154621" style="zoom:67%;" />
...
1. 集团里有 n 名员工，他们可以完成各种各样的工作创造利润。
   ...
   class Solution {
       public int profitableSchemes(int n, int minProfit, int[] group, int[] profit) {
           int len = group.length, mo = (int)1e9 + 7;
           int dp[][][] = new int[len + 1][n + 1][minProfit + 1];
           dp[0][0][0] = 1;
           for(int i = 1; i <= len; i++){
               int mem = group[i - 1], pro = profit[i - 1];
               for(int j = 0; j <= n; j++){
                   for(int k = 0; k <= minProfit; k++){
                       if(j < mem){
                           //代表第i个值遍历的时候，前面选择了j个人，且价值不低于k的个数
                           dp[i][j][k] = dp[i - 1][j][k];
                       } else {
                           dp[i][j][k] = (dp[i - 1][j][k] + dp[i - 1][j - mem][Math.max(0, k - pro)]) % mo;
                       }
                   }
               }
           }
...
1. 给一个总金额和一组硬币值，硬币数量无限，问可以有几种硬币数量组合：
...
1. 平方数：https://leetcode-cn.com/problems/perfect-squares/submissions/
...
1. 产品开发：https://leetcode-cn.com/problems/first-bad-version/

   找到第一个错误的版本，二分查找，但是要用l + 一半的办法，否则会越界

   **求一半的时候，害怕过界，所以用l + (r - l)**
...
1. 找出一个数组中超过一半数目的一个数字。众数的寻找方法，摩尔投票法，遍历数组，找一个候选，count，count为0就变为候选，相等count+1，不等count-1。最后为了确保数目多于一半所以要再次进行遍历，看得出的这个数字是否真的多于一半：
...
1. H指数。h 指数的定义：h 代表“高引用次数”（high citations），一名科研人员的 h 指数是指他（她）的 （N 篇论文中）总共有 h 篇论文分别被引用了至少 h 次。且其余的 N - h 篇论文每篇被引用次数 不超过 h 次。
   ...
   遇到序列可以想想排序。没看的很明白这题的意思。

2. 最小覆盖子串，找出s里面覆盖了t所有字母的最短子串
   ...
   滑动窗口，就是每次往右边走一个，然后检查是否符合要求，符合了就对左边进行移动，看其是否缩小之后仍然符合，这样达到缩小并枚举所有字符串的思路。

3. 找是否存在这个单词。

   https://leetcode-cn.com/problems/word-search/

   处理思路有问题，应该处理当前的而不是处理后一个字符
   ...
   class Solution {
       ...
       public boolean exist(char[][] board, String word) {
           ...
           rLen = board.length;
           cLen = board[0].length;
           if (rLen * cLen < word.length()) return false;
           vi = new int[rLen][cLen];
       ...
       void dfs(char[][] board, int i, int j, String word, int index) {
          ...
          if (i < 0 || i >= rLen || j < 0 || j >= cLen || board[i][j] != word.charAt(index)) {
              return ;
          }
          if (vi[i][j] == 0) {
           vi[i][j] = 1;
